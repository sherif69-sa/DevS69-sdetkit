...
| Real-world adoption and learning | [Adopt in your repository](adoption.md), [Rust adoption-to-diagnosis proof](rust-adoption-to-diagnosis-proof.md), [Artifact reference](artifact-reference.md), [Investigation operator guide](investigation-operator-guide.md) | Read-only external-repo evidence, complete ecosystem proofs, learning observations, detector upgrades, and roadmap/radar control panels. |
| Product direction | [Product roadmap](roadmap/product-roadmap.md), [Current product delta](current-product-delta.md) | Use the roadmap for future execution order and the delta for released-versus-main truth. Keep both secondary to the operator path. |
...
| Roadmap control panels | [Product roadmap](roadmap/product-roadmap.md), [Current product delta](current-product-delta.md), [Curated advanced docs material map](orphan-docs-material-map.md), [Artifact reference](artifact-reference.md) | Use the maintained roadmap to choose product verticals and radar/report outputs to choose focused PRs; do not let generated trackers replace product direction. |
