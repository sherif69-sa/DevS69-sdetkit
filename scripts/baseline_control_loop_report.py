#!/usr/bin/env python3
"""Summarize baseline progress by control-loop stage."""
...
def build_control_loop_report(
    ...
    checks.append(
        {
            "stage": "build",
            "ok": build_ok,
            "evidence": "baseline-summary exists",
            "action_if_missing": "Run make baseline-baseline",
        }
    )
    ...
    checks.append(
        {
            "stage": "validate",
            "ok": validate_ok,
            "evidence": "baseline-completion-dashboard exists",
            "action_if_missing": "Run make baseline-dashboard",
        }
    )
    ...
    checks.append(
        {
            ...
            "evidence": "baseline-weekly-pack exists",
            "action_if_missing": "Run make baseline-weekly-pack",
        }
    )
    ...
    checks.append(
        {
            ...
            "action_if_missing": "Run make baseline-next",
        }
    )
    ...
    return {
        "schema_version": "sdetkit.baseline_control_loop_report.v1",
        ...
        "ready_for_completion report": bool(dashboard.get("ready_to_close", False)),
...
def _to_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# baseline control loop report",
        "",
        f"- Generated at: {payload.get('generated_at', '')}",
        f"- Completion: {payload.get('completion_percent', 0)}%",
        f"- Ready for completion report: {payload.get('ready_for_completion report', False)}",
        "",
        "## Stage checks",
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build baseline control loop report.")
    parser.add_argument("--plan", default="plans/strategic-execution-model-2026.json")
    parser.add_argument("--baseline-summary", default="build/baseline/baseline-summary.json")
    parser.add_argument("--dashboard", default="build/baseline/baseline-completion-dashboard.json")
    parser.add_argument(
        "--weekly-pack", default="build/baseline/weekly-pack/baseline-weekly-pack.json"
    )
    parser.add_argument("--out-json", default="build/baseline/baseline-control-loop-report.json")
    parser.add_argument("--out-md", default="build/baseline/baseline-control-loop-report.md")
    ...
    if args.format == "json":
        ...
        print("baseline-control-loop-report: OK")
