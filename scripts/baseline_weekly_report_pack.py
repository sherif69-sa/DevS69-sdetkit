#!/usr/bin/env python3
"""Build a single weekly baseline report pack from generated artifacts."""
...
ARTIFACT_INPUTS = {
    "status": "build/baseline/baseline-status.json",
    "next_actions": "build/baseline/baseline-next-actions.json",
    "ops_snapshot": "build/baseline/baseline-ops-snapshot.json",
    "completion_dashboard": "build/baseline/baseline-completion-dashboard.json",
    "baseline_summary": "build/baseline/baseline-summary.json",
}
...
def build_pack_payload(inputs: dict[str, Path]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "sdetkit.baseline_weekly_report_pack.v1",
        ...
        "next_step": loaded.get("completion_dashboard", {}).get("next_step", "make baseline-next"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build baseline weekly report pack.")
    parser.add_argument("--out-dir", default="build/baseline/weekly-pack")
    ...
    out_json = out_dir / "baseline-weekly-pack.json"
    out_md = out_dir / "baseline-weekly-pack.md"
