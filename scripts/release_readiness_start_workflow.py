#!/usr/bin/env python3
"""Execute release readiness startup workflow and emit an operational summary."""
...
def build_summary(steps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "sdetkit.release_readiness_start_workflow.v1",
...
def _to_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# release readiness startup workflow summary",
        "",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run release readiness startup workflow.")
    parser.add_argument("--out-dir", default="build/release-readiness-start")
    ...
    kickoff_pack = workflow_root / "release readiness-kickoff-pack"
    steps = [
        _run_step(["python", "scripts/release_readiness_seed_prerequisites.py"]),
        _run_step(
            [
                "python",
                "-m",
    ...
    summary_json = out_dir / "release-readiness-start-summary.json"
    summary_md = out_dir / "release-readiness-start-summary.md"
    ...
    if args.format == "json":
        ...
        print(
            "release-readiness-start-workflow: OK"
            if summary["ok"]
            else "release-readiness-start-workflow: FAIL"
        )
