#!/usr/bin/env python3
"""Append baseline run telemetry and compute drift metrics."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Update baseline telemetry history.")
    parser.add_argument("--run-json", default="build/baseline/baseline-run-all.json")
    parser.add_argument("--history-json", default="build/baseline/baseline-telemetry-history.json")
    parser.add_argument("--summary-json", default="build/baseline/baseline-telemetry-summary.json")
    ...
    if not run_payload:
        result = {
            "ok": False,
            "schema_version": "sdetkit.baseline_telemetry_history.v1",
            "reason": f"missing run payload: {args.run_json}",
        }
        if args.format == "json":
            print(json.dumps(result, indent=2, sort_keys=True))
        else:
            print(f"baseline-telemetry-history: FAIL ({result['reason']})")
        return 1
    ...
    summary_payload = {
        "schema_version": "sdetkit.baseline_telemetry_summary.v1",
    ...
    if args.format == "json":
        ...
        print("baseline-telemetry-history: OK")
