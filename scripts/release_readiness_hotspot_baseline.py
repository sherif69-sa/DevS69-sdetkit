...
def build_payload(paths: list[Path]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "sdetkit.release-readiness-hotspot-baseline.v1",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build release readiness hotspot baseline metrics for repo.py and doctor.py."
    )
    parser.add_argument(
        "--paths",
        nargs="+",
    ...
    parser.add_argument(
        "--out",
        default="docs/artifacts/release-readiness-hotspot-baseline-2026-04-24.json",
