#!/usr/bin/env python3
"""Generate deterministic next actions from baseline status payload."""
...
def main() -> int:
    ...
    if not status_path.is_file():
        payload = {
            "ok": False,
            "schema_version": "sdetkit.baseline_next_actions.v1",
            "reason": f"missing status file: {status_path}",
            "next_actions": [
                "Run: python scripts/baseline_status_report.py --format json --out build/baseline/baseline-status.json"
            ],
        }
        if ns.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
            print("baseline-next-actions: FAIL")
    ...
    for item in not_yet:
        ...
        if text.startswith("python_version>=3.11"):
            next_actions.append(
                "Use Python 3.11+ for baseline baseline checks (core CLI runtime supports 3.10+) "
                "then rerun: make baseline-baseline"
            )
            ...
            next_actions.append("Generate baseline summary: make baseline-baseline")
    ...
    if ns.format == "json":
        ...
        print("baseline-next-actions: READY" if ok else "baseline-next-actions: ACTIONS_REQUIRED")
