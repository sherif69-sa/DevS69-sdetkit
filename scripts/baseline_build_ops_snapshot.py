#!/usr/bin/env python3
"""Build baseline weekly ops snapshot + quality debt register from baseline artifacts."""
        ...
        rows.append(
            {
                ...
                "recommendation": f"Address failing check '{check_id}' and rerun make baseline-baseline",
            }
        )
...
def build_ops_snapshot(
    ...
    return {
        "schema_version": "sdetkit.baseline_ops_snapshot.v1",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build baseline ops snapshot from baseline artifacts."
    )
    ...
    parser.add_argument("--out-json", default="build/baseline/baseline-ops-snapshot.json")
    parser.add_argument("--out-md", default="build/baseline/baseline-ops-snapshot.md")
    ...
    if args.format == "json":
        ...
        print("baseline-ops-snapshot: OK")
