...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate example asset contract.")
    parser.add_argument("--root", default=".")
    parser.add_argument("--skip-evidence", action="store_true")
    ns = parser.parse_args()

    root = Path(ns.root).resolve()
    payload = d33.build_example_asset_summary(root)
    ...
    if not ns.skip_evidence:
        evidence = (
            root / "docs/artifacts/example-asset-pack/evidence/example-execution-summary.json"
        )
