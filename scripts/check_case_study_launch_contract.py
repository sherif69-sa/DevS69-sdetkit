...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate case-study launch closeout contract")
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/case-study-launch-closeout-pack/evidence/case-study-launch-execution-summary.json"
        )
    ...
    if errors:
        print("case-study-launch-closeout contract check failed:", file=sys.stderr)
        for err in errors:
            print(f"- {err}", file=sys.stderr)
        return 1

    print("case-study-launch-closeout contract check passed")
    return 0
