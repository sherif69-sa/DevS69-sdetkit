#!/usr/bin/env python3
"""Compute a simple finish signal for baseline."""
...
def build_readiness_signal(
    control_loop: dict[str, Any], dashboard: dict[str, Any]
) -> dict[str, Any]:
    ...
    return {
        "schema_version": "sdetkit.baseline_readiness_signal.v1",
        "status": status,
        "completion_percent": completion,
        "ready_to_close": ready,
        "gate_ok": gate_ok,
        "blocking_required_checks": blockers,
        "next_step": dashboard.get("next_step", "make baseline-next"),
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compute baseline finish signal.")
    parser.add_argument(
    ...
    if not control or not dashboard:
        ...
        if args.format == "json":
            ...
            print(f"baseline-readiness-signal: FAIL ({payload['reason']})")
        return 1

    payload = build_readiness_signal(control, dashboard)
    payload["ok"] = True

    if args.format == "json":
        print(json.dumps(payload, indent=2, sort_keys=True))
    else:
        print(f"baseline-readiness-signal: {payload['status']}")
