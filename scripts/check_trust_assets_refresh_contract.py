...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate trust assets refresh closeout contract")
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/trust-assets-refresh-closeout-pack/evidence/trust-assets-refresh-execution-summary.json"
        )
    ...
    if errors:
        print("trust-assets-refresh-closeout contract check failed:", file=sys.stderr)
        for err in errors:
            print(f"- {err}", file=sys.stderr)
        return 1

    print("trust-assets-refresh-closeout contract check passed")
    return 0
