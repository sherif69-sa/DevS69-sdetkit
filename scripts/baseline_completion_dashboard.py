#!/usr/bin/env python3
"""Build a single readiness dashboard for baseline completion."""
...
def _sequential_summary(plan_path: Path, status_path: Path) -> dict[str, Any]:
    ...
    progress = {
        ...
        "baseline_ok": bool(status.get("ok", False)) if isinstance(status, dict) else False,
...
def build_dashboard(
    ...
    return {
        "schema_version": "sdetkit.baseline_completion_dashboard.v1",
        ...
        "next_step": "make baseline-completion-report"
        if ready
        else "make baseline-next && make baseline-ops-snapshot",
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build baseline completion dashboard.")
    parser.add_argument("--plan", default="plans/strategic-execution-model-2026.json")
    parser.add_argument("--status", default="build/baseline/baseline-status.json")
    parser.add_argument("--summary", default="build/baseline/baseline-summary.json")
    parser.add_argument("--snapshot", default="build/baseline/baseline-ops-snapshot.json")
    parser.add_argument("--out-json", default="build/baseline/baseline-completion-dashboard.json")
    parser.add_argument("--out-md", default="build/baseline/baseline-completion-dashboard.md")
    ...
    if args.format == "json":
        ...
        print("baseline-completion-dashboard: OK")
