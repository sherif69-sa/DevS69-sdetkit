...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Cycle 2 continuous upgrade closeout contract"
    )
    ...
    payload = c2.build_continuous_upgrade_cycle2_closeout_summary(root)
    errors: list[str] = []
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/continuous-upgrade-cycle2-closeout-pack/evidence/cycle2-execution-summary.json"
        )
    ...
    if errors:
        print("continuous-upgrade-cycle2-closeout contract check failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("continuous-upgrade-cycle2-closeout contract check passed")
    return 0
