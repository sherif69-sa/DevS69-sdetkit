#!/usr/bin/env python3
"""Build a concise executive report for current baseline status."""
...
def build_report(
    ...
    top_blockers = rows[:3]
    return {
        "schema_version": "sdetkit.baseline_executive_report.v1",
        "generated_at": _utc_now(),
        "status": finish.get("status", "unknown"),
        "ready_for_release readiness": bool(gate.get("ready_for_release readiness", False)),
        "completion_percent": float(finish.get("completion_percent", 0) or 0),
        "gate_next_step": gate.get("next_step", "make baseline-next"),
...
def _to_markdown(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Ready for release readiness: {payload.get('ready_for_release readiness', False)}",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build baseline executive report.")
    parser.add_argument("--finish", default="build/baseline/baseline-readiness-signal.json")
    parser.add_argument("--gate", default="build/baseline/baseline-release-readiness-gate.json")
    parser.add_argument("--blockers", default="build/baseline/baseline-blocker-register.json")
    parser.add_argument("--telemetry", default="build/baseline/baseline-telemetry-summary.json")
    parser.add_argument("--out-json", default="build/baseline/baseline-executive-report.json")
    parser.add_argument("--out-md", default="build/baseline/baseline-executive-report.md")
    ...
    if not finish or not gate:
        ...
        if args.format == "json":
            ...
            print(f"baseline-executive-report: FAIL ({payload['reason']})")
        return 1
    ...
    if args.format == "json":
        ...
        print(f"baseline-executive-report: {report.get('status', 'unknown')}")
        print(f"- ready_for_release readiness: {report.get('ready_for_release readiness', False)}")
