...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate example asset #2 contract.")
    parser.add_argument("--root", default=".")
    parser.add_argument("--skip-evidence", action="store_true")
    ns = parser.parse_args()

    root = Path(ns.root).resolve()
    payload = d34.build_example_asset2_summary(root)
    ...
    if not ns.skip_evidence:
        evidence = (
            root / "docs/artifacts/example-asset2-pack/evidence/demo-asset2-execution-summary.json"
        )
