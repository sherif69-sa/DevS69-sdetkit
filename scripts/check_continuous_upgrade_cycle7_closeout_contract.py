...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate continuous upgrade cycle 7 closeout contract"
    )
    ...
    payload = c7.build_continuous_upgrade_cycle7_closeout_summary(root)
    errors: list[str] = []
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/continuous-upgrade-cycle7-closeout-pack/evidence/continuous-upgrade-cycle7-execution-summary.json"
        )
    ...
    if errors:
        print("continuous-upgrade-cycle7-closeout contract check failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("continuous-upgrade-cycle7-closeout contract check passed")
    return 0
