...
from sdetkit import integration_expansion_64 as d64


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate integration expansion closeout contract")
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/integration-expansion-closeout-pack/evidence/integration-expansion-execution-summary.json"
        )
    ...
    if errors:
        print("integration-expansion-closeout contract check failed:", file=sys.stderr)
        for err in errors:
            print(f"- {err}", file=sys.stderr)
        return 1

    print("integration-expansion-closeout contract check passed")
    return 0
