#!/usr/bin/env python3
"""Validate that documented baseline flow commands map to Makefile targets."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Check baseline flow contract between docs and Makefile."
    )
    ...
    if not doc_path.is_file() or not make_path.is_file():
        payload = {
            "ok": False,
            "schema_version": "sdetkit.baseline_flow_contract.v1",
            "reason": "missing doc or makefile",
        }
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
            print(f"baseline-flow-contract: FAIL ({payload['reason']})")
        return 1
    ...
    if args.format == "json":
        ...
        print("baseline-flow-contract: OK" if ok else "baseline-flow-contract: FAIL")
