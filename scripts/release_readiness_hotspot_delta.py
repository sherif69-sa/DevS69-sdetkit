...
def build_delta(*, baseline: dict[str, Any], current: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "sdetkit.release-readiness-hotspot-delta.v1",
...
def _to_markdown(delta: dict[str, Any]) -> str:
    lines = [
        "# release readiness Hotspot Delta",
        "",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build release readiness hotspot delta from baseline/current metrics."
    )
