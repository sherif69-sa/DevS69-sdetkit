...
def build_continuous_upgrade_cycle1_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-1.json to restore required keys."
        )
    ...
    if not plan_trajectory_issues:
        ...
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-1.json target metrics so each numeric target is >= baseline."
        )
    ...
    if not plan_owner_issues:
        ...
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-1.json."
        )
    ...
    if not plan_hygiene_issues:
        ...
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-plan-1.json."
        )
