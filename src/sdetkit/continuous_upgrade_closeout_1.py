...
def _validate_plan_contract(
    ...
    cadence_interval = plan_data.get("cadence_interval", 7)
...
def build_continuous_upgrade_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "passed": (
                ...
                and "integrations-continuous-upgrade-closeout-1.md" in docs_index_text
            ),
    ...
    if not plan_hygiene_issues:
        ...
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_interval bounds in docs/roadmap/plans/continuous-upgrade-plan-1.json."
        )
    ...
    return {
        "name": "continuous-upgrade-closeout-1",
        "inputs": {
            "readme": "README.md",
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-closeout-1-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-closeout-1-summary.md", _render_text(payload) + "\n")
    _write(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-closeout-1-pack/evidence")
        )
