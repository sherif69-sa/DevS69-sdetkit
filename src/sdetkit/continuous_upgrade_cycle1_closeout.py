...
Cycle 1 weights continuity + execution contract + governance artifact readiness for a 100-point activation score.
"""
...
def build_continuous_upgrade_cycle1_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "weight": 5,
            "passed": ("continuous-upgrade-cycle1-closeout" in readme_text),
        ...
        },
        {
            ...
            "passed": (
                ...
                and "integrations-continuous-upgrade-cycle1-closeout.md" in docs_index_text
            ),
            "evidence": "continuous-upgrade-cycle1-big-upgrade-report.md + integrations-continuous-upgrade-cycle1-closeout.md",
        },
        {
        ...
        },
        {
            ...
            "weight": 7,
            "passed": _page_header_ok(page_lines, 1),
    ...
    if not missing_plan_keys:
        ...
        misses.append("Cycle 1 continuous upgrade dataset is missing required keys.")
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-cycle1-plan.json to restore required keys."
        )

    if not plan_trajectory_issues:
        wins.append("Cycle 1 target metrics are non-regressive against baseline metrics.")
    else:
        misses.append("Cycle 1 target metrics regress against baseline metrics.")
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-cycle1-plan.json target metrics so each numeric target is >= baseline."
        )

    if not plan_owner_issues:
        wins.append("Cycle 1 owner coverage includes both execution and rollback ownership.")
    else:
        misses.append("Cycle 1 owner coverage is missing execution and/or rollback ownership.")
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-cycle1-plan.json."
        )

    if not plan_hygiene_issues:
        wins.append(
            "Cycle 1 plan hygiene checks passed for contributors/channels/confidence/cadence."
        )
    else:
        misses.append(
            "Cycle 1 plan hygiene checks failed for contributors/channels/confidence/cadence."
        )
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-cycle1-plan.json."
        )

    if not failed and not critical_failures:
        wins.append(
            "Cycle 1 continuous upgrade closeout lane is fully complete and ready for continuous-upgrade backlog execution."
        )

    score = int(round(sum(c["weight"] for c in checks if c["passed"])))
    return {
        "name": "continuous-upgrade-cycle1-closeout",
        "inputs": {
            "readme": "README.md",
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-cycle1-closeout-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle1-closeout-summary.md", _render_text(payload) + "\n")
    _write(
        target / "continuous-upgrade-cycle1-evidence-brief.md",
        "# Cycle 1 continuous upgrade brief\n",
    )
    _write(target / "continuous-upgrade-cycle1-plan.md", "# Cycle 1 continuous upgrade plan\n")
    _write(
        target / "continuous-upgrade-cycle1-upgrade-template-upgrade-ledger.json",
        json.dumps({"upgrades": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle1-storyline-outcomes-ledger.json",
        json.dumps({"outcomes": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle1-upgrade-kpi-scorecard.json",
        json.dumps({"kpis": []}, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle1-execution-log.md", "# Cycle 1 execution log\n")
    _write(
        target / "continuous-upgrade-cycle1-delivery-board.md",
        "\n".join(["# Cycle 1 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle1-validation-commands.md",
...
def _execute_commands(root: Path, evidence_dir: Path) -> dict[str, Any]:
    ...
    _write(
        out_dir / "cycle1-execution-summary.json",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Cycle 1 continuous upgrade closeout checks")
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-cycle1-closeout-pack/evidence")
        )
