...
_DAY68_BOARD_PATH = "docs/artifacts/day68-integration-expansion4-closeout-pack/day68-delivery-board.md"
...
def build_day69_case_study_prep1_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {"check_id": "readme_day69_command", "weight": 7, "passed": ("day69-case-study-prep1-closeout" in readme_text), "evidence": "README day69 command lane"},
        {
            "check_id": "docs_index_day69_links",
            "weight": 8,
            "passed": ("day-69-big-upgrade-report.md" in docs_index_text and "integrations-day69-case-study-prep1-closeout.md" in docs_index_text),
            "evidence": "day-69-big-upgrade-report.md + integrations-day69-case-study-prep1-closeout.md",
        },
        {"check_id": "top10_day69_alignment", "weight": 5, "passed": ("Day 69" in top10_text and "Day 70" in top10_text), "evidence": "Day 69 + Day 70 strategy chain"},
        {"check_id": "day68_summary_present", "weight": 10, "passed": day68_summary.exists(), "evidence": str(day68_summary)},
        {"check_id": "day68_delivery_board_present", "weight": 7, "passed": day68_board.exists(), "evidence": str(day68_board)},
        {
            "check_id": "day68_quality_floor",
            "weight": 13,
            "passed": day68_strict and day68_score >= 95,
            "evidence": {"day68_score": day68_score, "strict_pass": day68_strict, "day68_checks": day68_check_count},
        },
        {
    ...
    if day68_strict:
        ...
        handoff_actions.append("Re-run Day 68 closeout command and restore strict baseline before Day 69 lock.")

    if board_count >= 5 and board_has_day68:
        wins.append(f"Day 68 delivery board integrity validated with {board_count} checklist items.")
    else:
        misses.append("Day 68 delivery board integrity is incomplete (needs >=5 items and Day 68 anchors).")
        handoff_actions.append("Repair Day 68 delivery board entries to include Day 68 anchors.")

    if not missing_case_data_keys:
        wins.append("Day 69 reliability case-study dataset is available for case-study prep execution.")
    else:
        misses.append("Day 69 reliability case-study dataset is missing required keys.")
        handoff_actions.append("Update .day69-reliability-case-study.json to restore required keys.")

    if not failed and not critical_failures:
        wins.append("Day 69 case-study prep #1 closeout lane is fully complete and ready for Day 70 case-study prep #2.")
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": _TOP10_PATH,
            "day68_summary": str(day68_summary.relative_to(root)) if day68_summary.exists() else str(day68_summary),
            "day68_delivery_board": str(day68_board.relative_to(root)) if day68_board.exists() else str(day68_board),
            "case_study_data": _CASE_STUDY_DATA_PATH,
        },
        "checks": checks,
        "rollup": {"day68_activation_score": day68_score, "day68_checks": day68_check_count, "day68_delivery_board_items": board_count},
        "summary": {
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(target / "day69-case-study-prep1-closeout-summary.json", json.dumps(payload, indent=2) + "\n")
    ...
    _write(target / "day69-delivery-board.md", "\n".join(["# Day 69 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n")
    _write(target / "day69-validation-commands.md", "# Day 69 validation commands\n\n```bash\n" + "\n".join(_EXECUTION_COMMANDS) + "\n```\n")
...
def _execute_commands(root: Path, evidence_dir: Path) -> None:
    ...
    _write(out_dir / "day69-execution-summary.json", json.dumps({"total_commands": len(events), "commands": events}, indent=2) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = Path(ns.evidence_dir) if ns.evidence_dir else Path("docs/artifacts/day69-case-study-prep1-closeout-pack/evidence")
