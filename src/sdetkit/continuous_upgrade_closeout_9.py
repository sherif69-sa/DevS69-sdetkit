...
_PREV_CYCLE_SUMMARY_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-8/continuous-upgrade-closeout-summary-8.json"
_PREV_CYCLE_BOARD_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-8/continuous-upgrade-delivery-board-8.md"
_PLAN_PATH = "docs/roadmap/plans/continuous-upgrade-plan-9.json"
...
_REQUIRED_COMMANDS = [
    ...
    "python -m sdetkit continuous-upgrade-closeout-9 --execute --evidence-dir docs/artifacts/continuous-upgrade-closeout-9-pack/evidence --format json --strict",
    "python scripts/check_continuous_upgrade_contract_9.py",
]
_EXECUTION_COMMANDS = [
    "python -m sdetkit continuous-upgrade-closeout-9 --format json --strict",
    "python -m sdetkit continuous-upgrade-closeout-9 --emit-pack-dir docs/artifacts/continuous-upgrade-closeout-9-pack --format json --strict",
    "python scripts/check_continuous_upgrade_contract_9.py --skip-evidence",
]
...
- `docs/roadmap/plans/continuous-upgrade-plan-9.json`
...
def build_continuous_upgrade_cycle9_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    board_has_cycle8 = "cycle 8" in board_text.lower()
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "weight": 5,
            "passed": ("continuous-upgrade-closeout-9" in readme_text),
        ...
        },
        {
            ...
            "passed": (
                ...
                and "integrations-continuous-upgrade-closeout-9.md" in docs_index_text
            ),
            "evidence": "continuous-upgrade-cycle9-big-upgrade-report.md + integrations-continuous-upgrade-closeout-9.md",
        },
        {
            "check_id": "top10_cycle9_align",
            "weight": 5,
            "passed": ("cycle 8" in top10_text.lower() and "cycle 9" in top10_text.lower()),
            "evidence": "Cycle 8 + Cycle 9 strategy chain",
        },
        {
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-9.json to restore required keys."
        )
    ...
    if not plan_trajectory_issues:
        ...
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-9.json target metrics so each numeric target is >= baseline."
        )
    ...
    if not plan_owner_issues:
        ...
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-9.json."
        )
    ...
    if not plan_hygiene_issues:
        ...
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-plan-9.json."
        )
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-closeout-9-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-closeout-9-summary.md", _render_text(payload) + "\n")
    _write(
        target / "continuous-upgrade-cycle9-evidence-brief.md",
        "# Cycle 9 continuous upgrade brief\n",
    )
    _write(target / "continuous-upgrade-cycle9-plan.md", "# Cycle 9 continuous upgrade plan\n")
    _write(
        target / "continuous-upgrade-cycle9-upgrade-template-upgrade-ledger.json",
        json.dumps({"upgrades": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle9-storyline-outcomes-ledger.json",
        json.dumps({"outcomes": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle9-upgrade-kpi-scorecard.json",
        json.dumps({"kpis": []}, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle9-execution-log.md", "# Cycle 9 execution log\n")
    _write(
        target / "continuous-upgrade-cycle9-delivery-board.md",
        "\n".join(["# Cycle 9 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle9-validation-commands.md",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-closeout-9-pack/evidence")
        )
