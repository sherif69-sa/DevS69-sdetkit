...
def _load_cycle50(path: Path) -> tuple[float, bool, int]:
...
def _resolve_cycle50_inputs(root: Path) -> tuple[Path, Path]:
...
def build_case_snippet_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    cycle50_summary, cycle50_board = _resolve_cycle50_inputs(root)
    cycle50_score, cycle50_strict, cycle50_check_count = _load_cycle50(cycle50_summary)
    board_count, board_has_cycle50, board_has_cycle51 = _board_stats(cycle50_board)
    ...
    checks: list[dict[str, Any]] = [
        ...
        {
            "check_id": "cycle50_summary_present",
            "weight": 10,
            "passed": cycle50_summary.exists(),
            "evidence": str(cycle50_summary),
        },
        {
            "check_id": "cycle50_delivery_board_present",
            "weight": 8,
            "passed": cycle50_board.exists(),
            "evidence": str(cycle50_board),
        },
        {
            "check_id": "cycle50_quality_floor",
            "weight": 10,
            "passed": cycle50_strict and cycle50_score >= 95,
            "evidence": {
                "cycle50_score": cycle50_score,
                "strict_pass": cycle50_strict,
                "cycle50_checks": cycle50_check_count,
            },
        },
        {
            "check_id": "cycle50_board_integrity",
            "weight": 7,
    ...
    if not cycle50_summary.exists() or not cycle50_board.exists():
        critical_failures.append("cycle50_handoff_inputs")
    if not cycle50_strict:
        critical_failures.append("cycle50_strict_baseline")
    ...
    if cycle50_strict:
        wins.append(f"Day 50 continuity is strict-pass with activation score={cycle50_score}.")
    else:
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": top10_path,
            "cycle50_summary": str(cycle50_summary.relative_to(root))
            if cycle50_summary.exists()
            else str(cycle50_summary),
            "cycle50_delivery_board": str(cycle50_board.relative_to(root))
            if cycle50_board.exists()
            else str(cycle50_board),
        },
        "checks": checks,
        "rollup": {
            "cycle50_activation_score": cycle50_score,
            "cycle50_checks": cycle50_check_count,
            "cycle50_delivery_board_items": board_count,
        },
        "summary": {
...
def _render_text(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Day 50 activation score: `{payload['rollup']['cycle50_activation_score']}`",
        f"- Day 50 checks evaluated: `{payload['rollup']['cycle50_checks']}`",
        f"- Day 50 delivery board checklist items: `{payload['rollup']['cycle50_delivery_board_items']}`",
    ]
    if payload["wins"]:
        lines.append("- Wins:")
