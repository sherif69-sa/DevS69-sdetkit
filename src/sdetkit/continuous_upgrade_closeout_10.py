...
_PREV_CYCLE_SUMMARY_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-9/continuous-upgrade-closeout-summary-9.json"
_PREV_CYCLE_BOARD_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-9/continuous-upgrade-delivery-board-9.md"
_PLAN_PATH = "docs/roadmap/plans/continuous-upgrade-plan-10.json"
...
_REQUIRED_COMMANDS = [
    ...
    "python scripts/check_continuous_upgrade_contract_10.py",
]
_EXECUTION_COMMANDS = [
    "python -m sdetkit continuous-upgrade-closeout-10 --format json --strict",
    "python -m sdetkit continuous-upgrade-closeout-10 --emit-pack-dir docs/artifacts/continuous-upgrade-closeout-10-pack --format json --strict",
    "python scripts/check_continuous_upgrade_contract_10.py --skip-evidence",
]
...
- `docs/roadmap/plans/continuous-upgrade-plan-10.json`
...
def build_continuous_upgrade_cycle10_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "weight": 5,
            "passed": ("continuous-upgrade-closeout-10" in readme_text),
        ...
        },
        {
            ...
            "passed": (
                ...
                and "integrations-continuous-upgrade-closeout-10.md" in docs_index_text
            ),
            "evidence": "continuous-upgrade-cycle10-big-upgrade-report.md + integrations-continuous-upgrade-closeout-10.md",
        },
        {
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-10.json to restore required keys."
        )
    ...
    if not plan_trajectory_issues:
        ...
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-10.json target metrics so each numeric target is >= baseline."
        )
    ...
    if not plan_owner_issues:
        ...
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-10.json."
        )
    ...
    if not plan_hygiene_issues:
        ...
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-plan-10.json."
        )
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-closeout-10-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-closeout-10-summary.md", _render_text(payload) + "\n")
    _write(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-closeout-10-pack/evidence")
        )
