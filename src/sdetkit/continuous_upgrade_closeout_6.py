...
_CYCLE5_SUMMARY_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-5/continuous-upgrade-closeout-summary-5.json"
_CYCLE5_BOARD_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-5/continuous-upgrade-delivery-board-5.md"
_PLAN_PATH = "docs/roadmap/plans/continuous-upgrade-plan-6.json"
...
_REQUIRED_COMMANDS = [
    ...
    "python scripts/check_continuous_upgrade_contract_6.py",
]
_EXECUTION_COMMANDS = [
    "python -m sdetkit continuous-upgrade-cycle6-closeout --format json --strict",
    "python -m sdetkit continuous-upgrade-cycle6-closeout --emit-pack-dir docs/artifacts/continuous-upgrade-cycle6-closeout-pack --format json --strict",
    "python scripts/check_continuous_upgrade_contract_6.py --skip-evidence",
]
...
_CYCLE6_DEFAULT_PAGE = """# Cycle 6 \u2014 Continuous upgrade closeout lane
...
- `docs/roadmap/plans/continuous-upgrade-plan-6.json`
...
def build_continuous_upgrade_cycle6_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {
            "check_id": "readme_cycle6_command",
            "weight": 5,
            "passed": ("continuous-upgrade-cycle6-closeout" in readme_text),
            "evidence": "README cycle6 command lane",
        },
        {
            "check_id": "docs_index_cycle6_links",
            "weight": 8,
            "passed": (
                "integrations-continuous-upgrade-cycle6-closeout.md" in docs_index_text
                and "continuous-upgrade-cycle6-closeout-summary.json" in docs_index_text
            ),
            "evidence": "integrations-continuous-upgrade-cycle6-closeout.md",
        },
        {
            "check_id": "top10_cycle6_align",
            "weight": 5,
            "passed": ("cycle 5" in top10_text.lower() and "cycle 6" in top10_text.lower()),
    ...
    if board_count >= 5 and board_has_cycle5:
        ...
        misses.append(
            "Cycle 5 delivery board integrity is incomplete (needs >=5 items and cycle 5 anchors)."
        )
        handoff_actions.append("Repair cycle 5 delivery board entries to include cycle 5 anchors.")

    if not missing_plan_keys:
        wins.append("Cycle 6 continuous upgrade dataset is available for governance execution.")
    else:
        misses.append("Cycle 6 continuous upgrade dataset is missing required keys.")
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-6.json to restore required keys."
        )

    if not plan_trajectory_issues:
        wins.append("Cycle 6 target metrics are non-regressive against baseline metrics.")
    else:
        misses.append("Cycle 6 target metrics regress against baseline metrics.")
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-6.json target metrics so each numeric target is >= baseline."
        )

    if not plan_owner_issues:
        wins.append("Cycle 6 owner coverage includes both execution and rollback ownership.")
    else:
        misses.append("Cycle 6 owner coverage is missing execution and/or rollback ownership.")
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-6.json."
        )

    if not plan_hygiene_issues:
        wins.append(
            "Cycle 6 plan hygiene checks passed for contributors/channels/confidence/cadence."
        )
    else:
        misses.append(
            "Cycle 6 plan hygiene checks failed for contributors/channels/confidence/cadence."
        )
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-plan-6.json."
        )

    if not failed and not critical_failures:
        wins.append(
            "Continuous upgrade cycle 6 closeout lane is fully complete and ready for continuous-upgrade backlog execution."
        )

    score = int(round(sum(c["weight"] for c in checks if c["passed"])))
    return {
        "name": "continuous-upgrade-cycle6-closeout",
        "inputs": {
            "readme": "README.md",
...
def _render_text(payload: dict[str, Any]) -> str:
    lines = [
        "Continuous upgrade cycle 6 closeout summary",
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-cycle6-closeout-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle6-closeout-summary.md", _render_text(payload) + "\n")
    _write(
        target / "continuous-upgrade-cycle6-evidence-brief.md",
        "# Cycle 6 continuous upgrade brief\n",
    )
    _write(target / "continuous-upgrade-cycle6-plan.md", "# Cycle 6 continuous upgrade plan\n")
    _write(
        target / "continuous-upgrade-cycle6-template-ledger.json",
        json.dumps({"upgrades": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle6-storyline-outcomes-ledger.json",
        json.dumps({"outcomes": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle6-kpi-scorecard.json",
        json.dumps({"kpis": []}, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle6-execution-log.md", "# Cycle 6 execution log\n")
    _write(
        target / "continuous-upgrade-cycle6-delivery-board.md",
        "\n".join(["# Cycle 6 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle6-validation-commands.md",
...
def _execute_commands(root: Path, evidence_dir: Path) -> dict[str, Any]:
    ...
    _write(
        out_dir / "continuous-upgrade-cycle6-execution-summary.json",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Continuous upgrade cycle 6 closeout checks")
    ...
    if ns.write_default_doc:
        _write(root / _PAGE_PATH, _CYCLE6_DEFAULT_PAGE)

    payload = build_continuous_upgrade_cycle6_closeout_summary(root)
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-cycle6-closeout-pack/evidence")
        )
