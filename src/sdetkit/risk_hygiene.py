...
def classify_risks(root: Path, risks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for risk in risks:
        ...
        release_package = ('release' in signal or 'package' in signal or 'release' in title or 'package' in title)
        entry = dict(risk)
        ...
        if tracked_generated or release_package:
            ...
            reasons['untracked_generated_artifact'] = reasons.get('untracked_generated_artifact', 0) + 1
    return {
        'source_risks': source,
