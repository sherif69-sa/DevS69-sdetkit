...
_PREV_CYCLE_SUMMARY_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-10/continuous-upgrade-closeout-summary-10.json"
_PREV_CYCLE_BOARD_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-10/continuous-upgrade-delivery-board-10.md"
_PLAN_PATH = "docs/roadmap/plans/continuous-upgrade-plan-11.json"
...
_REQUIRED_COMMANDS = [
    ...
    "python scripts/check_continuous_upgrade_contract_11.py",
]
_EXECUTION_COMMANDS = [
    "python -m sdetkit continuous-upgrade-closeout-11 --format json --strict",
    "python -m sdetkit continuous-upgrade-closeout-11 --emit-pack-dir docs/artifacts/continuous-upgrade-closeout-11-pack --format json --strict",
    "python scripts/check_continuous_upgrade_contract_11.py --skip-evidence",
]
...
def build_continuous_upgrade_cycle11_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    board_has_cycle10 = "cycle 10" in board_text.lower()
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "weight": 5,
            "passed": ("continuous-upgrade-closeout-11" in readme_text),
        ...
        },
        {
            ...
            "passed": (
                ...
                and "integrations-continuous-upgrade-closeout-11.md" in docs_index_text
            ),
            "evidence": "continuous-upgrade-cycle11-big-upgrade-report.md + integrations-continuous-upgrade-closeout-11.md",
        },
        {
            "check_id": "top10_cycle11_align",
            "weight": 5,
            "passed": (
                ("Cycle 10" in top10_text or "Cycle 10" in top10_text) and "Cycle 11" in top10_text
            ),
            "evidence": "Cycle 10 + Cycle 11 strategy chain",
        },
        {
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-11.json to restore required keys."
        )
    ...
    if not plan_trajectory_issues:
        ...
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-11.json target metrics so each numeric target is >= baseline."
        )
    ...
    if not plan_owner_issues:
        ...
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-11.json."
        )
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-closeout-11-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-closeout-11-summary.md", _render_text(payload) + "\n")
    _write(
        target / "continuous-upgrade-cycle11-evidence-brief.md",
        "# Cycle 11 continuous upgrade brief\n",
    )
    _write(target / "continuous-upgrade-cycle11-plan.md", "# Cycle 11 continuous upgrade plan\n")
    _write(
        target / "continuous-upgrade-cycle11-upgrade-template-upgrade-ledger.json",
        json.dumps({"upgrades": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle11-storyline-outcomes-ledger.json",
        json.dumps({"outcomes": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle11-upgrade-kpi-scorecard.json",
        json.dumps({"kpis": []}, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle11-execution-log.md", "# Cycle 11 execution log\n")
    _write(
        target / "continuous-upgrade-cycle11-delivery-board.md",
        "\n".join(["# Cycle 11 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle11-validation-commands.md",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-closeout-11-pack/evidence")
        )
