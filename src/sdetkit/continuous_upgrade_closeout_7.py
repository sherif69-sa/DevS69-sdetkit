...
_CYCLE6_SUMMARY_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-6/continuous-upgrade-closeout-summary-6.json"
_CYCLE6_BOARD_PATH = "docs/artifacts/continuous-upgrade-closeout-pack-6/continuous-upgrade-delivery-board-6.md"
_PLAN_PATH = "docs/roadmap/plans/continuous-upgrade-plan-7.json"
...
_REQUIRED_COMMANDS = [
    ...
    "python scripts/check_continuous_upgrade_contract_7.py",
]
_EXECUTION_COMMANDS = [
    "python -m sdetkit continuous-upgrade-closeout-7 --format json --strict",
    "python -m sdetkit continuous-upgrade-closeout-7 --emit-pack-dir docs/artifacts/continuous-upgrade-closeout-7-pack --format json --strict",
    "python scripts/check_continuous_upgrade_contract_7.py --skip-evidence",
]
...
def build_continuous_upgrade_cycle7_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "weight": 5,
            "passed": ("continuous-upgrade-closeout-7" in readme_text),
        ...
        },
        {
            ...
            "weight": 8,
            "passed": (
                "integrations-continuous-upgrade-closeout-7.md" in docs_index_text
                and "continuous-upgrade-closeout-7-summary.json" in docs_index_text
            ),
            "evidence": "integrations-continuous-upgrade-closeout-7.md + continuous-upgrade-closeout-7-summary.json",
        },
        {
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-7.json to restore required keys."
        )
    ...
    if not plan_trajectory_issues:
        ...
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-7.json target metrics so each numeric target is >= baseline."
        )
    ...
    if not plan_owner_issues:
        ...
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-7.json."
        )
    ...
    if not plan_hygiene_issues:
        ...
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-plan-7.json."
        )
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(
        target / "continuous-upgrade-closeout-7-summary.json",
        json.dumps(payload, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-closeout-7-summary.md", _render_text(payload) + "\n")
    _write(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = (
            ...
            if ns.evidence_dir
            else Path("docs/artifacts/continuous-upgrade-closeout-7-pack/evidence")
        )
