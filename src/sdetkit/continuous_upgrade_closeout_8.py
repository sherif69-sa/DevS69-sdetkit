...
- `docs/artifacts/continuous-upgrade-closeout-pack-7/continuous-upgrade-closeout-summary-7.json`
- `docs/artifacts/continuous-upgrade-closeout-pack-7/continuous-upgrade-delivery-board-7.md`
- `docs/roadmap/plans/continuous-upgrade-plan-8.json`
...
def build_continuous_upgrade_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    board_has_cycle7 = "cycle 7" in board_text.lower()
    ...
    checks: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "weight": 5,
            "passed": ("cycle 7" in top10_text.lower() and "cycle 8" in top10_text.lower()),
            "evidence": "Cycle 7 + Cycle 8 strategy chain",
        },
        {
        ...
        },
        {
            ...
            "evidence": {"board_items": board_count, "contains_required_cycle": board_has_cycle7},
        },
        {
    ...
    if score >= 85 and strict:
        wins.append(f"Cycle 8 continuity baseline is stable with activation score={score}.")
    else:
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append(
            "Update docs/roadmap/plans/continuous-upgrade-plan-8.json to restore required keys."
        )
    ...
    if not plan_trajectory_issues:
        ...
        handoff_actions.append(
            "Adjust docs/roadmap/plans/continuous-upgrade-plan-8.json target metrics so each numeric target is >= baseline."
        )
    ...
    if not plan_owner_issues:
        ...
        handoff_actions.append(
            "Assign both owner and rollback_owner in docs/roadmap/plans/continuous-upgrade-plan-8.json."
        )
    ...
    if not plan_hygiene_issues:
        ...
        handoff_actions.append(
            "Fix contributors/upgrade_channels list shapes and confidence_floor/cadence_days bounds in docs/roadmap/plans/continuous-upgrade-plan-8.json."
        )
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(target / "continuous-upgrade-cycle8-plan.md", "# Cycle 8 continuous upgrade plan\n")
    _write(
        target / "continuous-upgrade-cycle8-upgrade-template-upgrade-ledger.json",
        json.dumps({"upgrades": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle8-storyline-outcomes-ledger.json",
        json.dumps({"outcomes": []}, indent=2) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle8-upgrade-kpi-scorecard.json",
        json.dumps({"kpis": []}, indent=2) + "\n",
    )
    _write(target / "continuous-upgrade-cycle8-execution-log.md", "# Cycle 8 execution log\n")
    _write(
        target / "continuous-upgrade-cycle8-delivery-board.md",
        "\n".join(["# Cycle 8 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n",
    )
    _write(
        target / "continuous-upgrade-cycle8-validation-commands.md",
