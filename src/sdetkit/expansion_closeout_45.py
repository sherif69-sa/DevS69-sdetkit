...
def build_expansion_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    scale_closeout_score, scale_closeout_strict, scale_closeout_check_count = _load_scale_closeout(
        scale_closeout_summary
    )
    board_count, board_has_scale_closeout, board_has_expansion_closeout = _board_stats(
        scale_closeout_board
    )
...
def _render_text(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"- 44 activation score: `{payload['rollup']['scale_closeout_activation_score']}`",
        f"- 44 checks evaluated: `{payload['rollup']['scale_closeout_checks']}`",
        f"- 44 delivery board checklist items: `{payload['rollup']['scale_closeout_delivery_board_items']}`",
    ]
    if payload["wins"]:
        lines.append("- Wins:")
