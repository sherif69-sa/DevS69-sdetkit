...
def build_day88_governance_priorities_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    day87_summary_data = day87_data.get("summary", {}) if isinstance(day87_data.get("summary"), dict) else {}
    day87_score = int(day87_summary_data.get("activation_score", 0) or 0)
    day87_strict = bool(day87_summary_data.get("strict_pass", False))
    day87_check_count = len(day87_data.get("checks", [])) if isinstance(day87_data.get("checks"), list) else 0
    ...
    checks: list[dict[str, Any]] = [
        {"check_id": "readme_day88_command", "weight": 7, "passed": ("day88-governance-priorities-closeout" in readme_text), "evidence": "README day88 command lane"},
        {
            "check_id": "docs_index_day88_links",
            "weight": 8,
            "passed": ("day-88-big-upgrade-report.md" in docs_index_text and "integrations-day88-governance-priorities-closeout.md" in docs_index_text),
            "evidence": "day-88-big-upgrade-report.md + integrations-day88-governance-priorities-closeout.md",
        },
        {"check_id": "top10_day88_alignment", "weight": 5, "passed": ("Day 87" in top10_text and "Day 88" in top10_text), "evidence": "Day 87 + Day 88 strategy chain"},
        {"check_id": "day87_summary_present", "weight": 10, "passed": day87_summary.exists(), "evidence": str(day87_summary)},
        {"check_id": "day87_delivery_board_present", "weight": 7, "passed": day87_board.exists(), "evidence": str(day87_board)},
        {
            "check_id": "day87_quality_floor",
            "weight": 13,
            "passed": day87_score >= 85 and day87_strict,
            "evidence": {"day87_score": day87_score, "strict_pass": day87_strict, "day87_checks": day87_check_count},
        },
        {
    ...
    if day87_score >= 85 and day87_strict:
        ...
        handoff_actions.append("Re-run Day 87 closeout command and raise baseline quality above 85 with strict pass before Day 88 lock.")

    if board_count >= 5 and board_has_day87:
        wins.append(f"Day 87 delivery board integrity validated with {board_count} checklist items.")
    else:
        misses.append("Day 87 delivery board integrity is incomplete (needs >=5 items and Day 87 anchors).")
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append("Update .day88-governance-priorities-plan.json to restore required keys.")

    if not failed and not critical_failures:
        wins.append("Day 88 governance priorities closeout lane is fully complete and ready for Day 89 governance planning.")
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": _TOP10_PATH,
            "day87_summary": str(day87_summary.relative_to(root)) if day87_summary.exists() else str(day87_summary),
            "day87_delivery_board": str(day87_board.relative_to(root)) if day87_board.exists() else str(day87_board),
            "governance_priorities_plan": _PLAN_PATH,
        },
        "checks": checks,
        "rollup": {"day87_activation_score": day87_score, "day87_checks": day87_check_count, "day87_delivery_board_items": board_count},
        "summary": {
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(target / "day88-governance-priorities-closeout-summary.json", json.dumps(payload, indent=2) + "\n")
    ...
    _write(target / "day88-narrative-template-upgrade-ledger.json", json.dumps({"upgrades": []}, indent=2) + "\n")
    _write(target / "day88-storyline-outcomes-ledger.json", json.dumps({"outcomes": []}, indent=2) + "\n")
    ...
    _write(target / "day88-delivery-board.md", "\n".join(["# Day 88 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n")
    _write(target / "day88-validation-commands.md", "# Day 88 validation commands\n\n```bash\n" + "\n".join(_EXECUTION_COMMANDS) + "\n```\n")
...
def _execute_commands(root: Path, evidence_dir: Path) -> None:
    ...
    _write(out_dir / "day88-execution-summary.json", json.dumps({"total_commands": len(events), "commands": events}, indent=2) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = Path(ns.evidence_dir) if ns.evidence_dir else Path("docs/artifacts/day88-governance-priorities-closeout-pack/evidence")
