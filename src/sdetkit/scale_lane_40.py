...
def build_scale_lane_summary_impl(
    ...
    *,
    ...
    cycle39_summary = root / _DAY39_SUMMARY_PATH
    cycle39_board = root / _DAY39_BOARD_PATH
    cycle39_score, cycle39_strict, cycle39_check_count = _load_cycle39(cycle39_summary)
    board_count, board_has_cycle39, board_has_cycle40 = _board_stats(cycle39_board)
    ...
    checks: list[dict[str, Any]] = [
        ...
        {
            "check_id": "cycle39_summary_present",
            "weight": 10,
            "passed": cycle39_summary.exists(),
            "evidence": str(cycle39_summary),
        },
        {
            "check_id": "cycle39_delivery_board_present",
            "weight": 8,
            "passed": cycle39_board.exists(),
            "evidence": str(cycle39_board),
        },
        {
            "check_id": "cycle39_quality_floor",
            "weight": 10,
            "passed": cycle39_strict and cycle39_score >= 95,
            "evidence": {
                "cycle39_score": cycle39_score,
                "strict_pass": cycle39_strict,
                "cycle39_checks": cycle39_check_count,
            },
        },
        {
            "check_id": "cycle39_board_integrity",
            "weight": 7,
    ...
    if not cycle39_summary.exists() or not cycle39_board.exists():
        critical_failures.append("cycle39_handoff_inputs")
    if not cycle39_strict:
        critical_failures.append("cycle39_strict_baseline")
    ...
    if cycle39_strict:
        wins.append(f"Day 39 continuity is strict-pass with activation score={cycle39_score}.")
    else:
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": top10_path,
            "cycle39_summary": str(cycle39_summary.relative_to(root))
            if cycle39_summary.exists()
            else str(cycle39_summary),
            "cycle39_delivery_board": str(cycle39_board.relative_to(root))
            if cycle39_board.exists()
            else str(cycle39_board),
        },
        "checks": checks,
        "rollup": {
            "cycle39_activation_score": cycle39_score,
            "cycle39_checks": cycle39_check_count,
            "cycle39_delivery_board_items": board_count,
        },
        "summary": {
...
def _to_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Day 39 continuity",
        "",
        f"- Day 39 activation score: `{payload['rollup']['cycle39_activation_score']}`",
        f"- Day 39 checks evaluated: `{payload['rollup']['cycle39_checks']}`",
        f"- Day 39 delivery board checklist items: `{payload['rollup']['cycle39_delivery_board_items']}`",
        "",
        "## Wins",
    ]
