...
from sdetkit import governance_scale as d89
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-governance-scale-workflow.md\ngovernance-scale-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-89-big-upgrade-report.md\nintegrations-governance-scale-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-governance-scale-workflow.md").write_text(
...
def test_governance_scale_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_governance_scale_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d89.main(
        [
...
def test_governance_scale_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_governance_scale_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
