...
from sdetkit import continuous_upgrade_closeout_5 as c5
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-continuous-upgrade-closeout-5.md\ncontinuous-upgrade-closeout-5\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-continuous-upgrade-closeout-5.md").write_text(
        c5._CYCLE5_DEFAULT_PAGE, encoding="utf-8"
    )
    (root / "docs/cycle-5-big-upgrade-report.md").write_text("# Cycle 5 report\n", encoding="utf-8")
    (root / "scripts").mkdir(parents=True, exist_ok=True)
    (root / "scripts/check_continuous_upgrade_contract_5.py").write_text(
        "from __future__ import annotations\n\nif __name__ == 'main_':\n    raise SystemExit(0)\n",
        encoding="utf-8",
    )
    ...
    plan = root / "docs/roadmap/plans/continuous-upgrade-plan-5.json"
    plan.write_text(
        json.dumps(
            {
...
def test_json(tmp_path: Path, capsys) -> None:
    ...
    assert out["name"] == "continuous-upgrade-closeout-5"
...
def test_emit_pack_and_execute(tmp_path: Path) -> None:
    ...
    rc = c5.main(
        [
            ...
            str(tmp_path),
            "--emit-pack-dir",
            "artifacts/continuous-upgrade-closeout-5-pack",
            "--execute",
            "--evidence-dir",
            "artifacts/continuous-upgrade-closeout-5-pack/evidence",
            "--format",
            "json",
            "--strict",
        ]
    ...
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-closeout-5-summary.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-evidence-brief.md"
    ).exists()
    assert (
        tmp_path / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-plan.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-upgrade-template-upgrade-ledger.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-storyline-outcomes-ledger.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-upgrade-kpi-scorecard.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-execution-log.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-delivery-board.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/continuous-upgrade-cycle5-validation-commands.md"
    ).exists()
    execution_summary = (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-5-pack/evidence/cycle5-execution-summary.json"
    )
...
def test_strict_fails_without_cycle4(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    (
        tmp_path
        / "docs/artifacts/continuous-upgrade-closeout-pack-4/continuous-upgrade-closeout-summary-4.json"
...
def test_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(["continuous-upgrade-closeout-5", "--root", str(tmp_path), "--format", "text"])
    assert rc == 0
