...
from sdetkit import optimization_foundation as d42
...
def test_optimization_foundation_optimization_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_optimization_foundation_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d42.main(
        [
...
def test_optimization_foundation_strict_fails_when_expansion_automation_inputs_missing(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_optimization_foundation_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
