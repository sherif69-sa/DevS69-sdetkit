...
def test_makefile_has_phase2_start_target() -> None:
    ...
    assert (
        "python scripts/phase2_status_report.py --format json --out build/phase2-start/phase2-status.json"
        in text
    )
    ...
    assert (
        "python scripts/phase2_progress_report.py --format json --out build/phase2-complete/phase2-progress.json"
        in text
    )
