...
from sdetkit import trust_assets_refresh as d75
from sdetkit.evidence import trust_assets_refresh as d75_impl
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-trust-assets-refresh-workflow.md\ntrust-assets-refresh-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-75-big-upgrade-report.md\nintegrations-trust-assets-refresh-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-trust-assets-refresh-workflow.md").write_text(
    ...
    (root / "scripts/check_trust_assets_refresh_contract.py").write_text(
        "from __future__ import annotations\n\n"
        "import sys\n\n"
...
def test_trust_assets_refresh_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_trust_assets_refresh_default_template_has_no_unresolved_placeholders() -> None:
...
def test_trust_assets_refresh_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d75.main(
        [
...
def test_trust_assets_refresh_strict_fails_without_distribution_scaling_baseline(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_trust_assets_refresh_board_anchor_is_enforced(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    board = (
        tmp_path
...
def test_trust_assets_refresh_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_trust_assets_refresh_execute_failure_returns_nonzero_and_reports_failure(
