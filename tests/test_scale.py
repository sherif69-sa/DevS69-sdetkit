...
from sdetkit import scale as d44
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-scale-workflow.md\nscale-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-44-big-upgrade-report.md\nintegrations-scale-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-scale-workflow.md").write_text(
...
def test_scale_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_scale_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d44.main(
        [
...
def test_scale_strict_fails_when_acceleration_inputs_missing(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
...
def test_scale_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_scale_alignment_checks_fail_when_top10_and_board_missing_markers(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    ...
    summary = d44.build_scale_summary(tmp_path)
    checks = {check["check_id"]: check for check in summary["checks"]}

    assert checks["top10_scale_alignment"]["passed"] is False
    assert checks["acceleration_board_integrity"]["passed"] is False
