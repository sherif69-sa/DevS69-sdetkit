...
_SCRIPT_PATH = (
    Path(__file__).resolve().parents[1] / "scripts" / "release_readiness_hotspot_delta.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "release_readiness_hotspot_delta_script", _SCRIPT_PATH
)
assert _SPEC is not None and _SPEC.loader is not None
release_readiness_delta = importlib.util.module_from_spec(_SPEC)
_SPEC.loader.exec_module(release_readiness_delta)
...
def test_build_delta_computes_module_differences() -> None:
    ...
    delta = release_readiness_delta.build_delta(baseline=baseline, current=current)
...
def test_main_writes_json_and_markdown(tmp_path: Path) -> None:
    ...
    rc = release_readiness_delta.main(
        [
            "--baseline",
            str(baseline),
            "--current",
