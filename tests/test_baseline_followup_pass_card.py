...
from scripts import baseline_followup_pass_card as card
...
def test_build_followup_pass_card() -> None:
    out = card.build_followup_pass_card(
        {
            "status": "near_finish",
...
def test_main_writes_outputs(tmp_path: Path) -> None:
    ...
    _write(
        finish,
        {
            ...
            "next_step": "make baseline-next",
        },
    )
    _write(nxt, {"next_actions": ["make baseline-next"]})
