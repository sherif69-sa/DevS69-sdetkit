...
def test_phase3_wrap_publication_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_phase3_wrap_publication_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d90.main(
        [
...
def test_phase3_wrap_publication_strict_fails_without_governance_scale(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
