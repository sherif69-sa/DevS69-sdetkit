...
from sdetkit import governance_priorities as d88
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-governance-priorities-workflow.md\ngovernance-priorities-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-88-big-upgrade-report.md\nintegrations-governance-priorities-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-governance-priorities-workflow.md").write_text(
...
def test_governance_priorities_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_governance_priorities_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d88.main(
        [
...
def test_governance_priorities_strict_fails_without_prereq_baseline(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_governance_priorities_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
