...
def test_formatter_policy_proposal_review_packet_uses_portable_verifier_paths() -> None:
    ...
    expected = {
        ...
        "replay_report": (PACKET_ROOT / "verifier" / "formatter-replay-report.json").as_posix(),
        "repo_memory": (PACKET_ROOT / "verifier" / "formatter-repo-memory.json").as_posix(),
        "trajectory": (PACKET_ROOT / "verifier" / "formatter-trajectory.jsonl").as_posix(),
    }
