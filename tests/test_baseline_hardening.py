...
def test_baseline_hardening_hardening_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_baseline_hardening_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    rc = d29.main(
        [
            "--root",
...
def test_baseline_hardening_strict_fails_when_sections_missing(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
...
def test_baseline_hardening_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(["baseline-hardening", "--root", str(tmp_path), "--format", "text"])
    assert rc == 0
    assert " baseline hardening summary" in capsys.readouterr().out
