...
from sdetkit import evidence_narrative as d84
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-evidence-narrative-workflow.md\nevidence-narrative-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-84-big-upgrade-report.md\nintegrations-evidence-narrative-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-evidence-narrative-workflow.md").write_text(
...
def test_evidence_narrative_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_evidence_narrative_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d84.main(
        [
...
def test_evidence_narrative_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_evidence_narrative_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
