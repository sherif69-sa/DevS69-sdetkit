...
from sdetkit import ecosystem_priorities as d78
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-ecosystem-priorities-workflow.md\necosystem-priorities-closeout\n",
        encoding="utf-8",
    )
    (root / "docs/index.md").write_text(
        "impact-78-big-upgrade-report.md\nintegrations-ecosystem-priorities-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-ecosystem-priorities-workflow.md").write_text(
...
def test_ecosystem_priorities_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_ecosystem_priorities_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d78.main(
        [
...
def test_ecosystem_priorities_strict_fails_without_community_touchpoint_summary(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_ecosystem_priorities_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
