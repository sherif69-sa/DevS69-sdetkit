...
from sdetkit import cli, enterprise_readiness
...
def test_enterprise_readiness_default_text(capsys):
    rc = enterprise_readiness.main([])
    assert rc == 0
...
def test_enterprise_readiness_help_output_is_productized():
    out = _normalize_ws(enterprise_readiness._build_parser().format_help())
...
def test_enterprise_readiness_markdown_output_uses_productized_headings(capsys):
    rc = enterprise_readiness.main(["--format", "markdown"])
    assert rc == 0
    out = capsys.readouterr().out
    assert "# Enterprise readiness report" in out
    assert "## Required sections" in out
...
def test_enterprise_readiness_json_and_strict_success(capsys):
    rc = enterprise_readiness.main(["--format", "json", "--strict"])
    assert rc == 0
...
def test_enterprise_readiness_strict_fails_when_content_missing(tmp_path, capsys):
    (tmp_path / "docs").mkdir(parents=True)
    (tmp_path / "docs/use-cases-enterprise-regulated.md").write_text(
        "# Placeholder\n", encoding="utf-8"
    )
    rc = enterprise_readiness.main(["--root", str(tmp_path), "--strict"])
    assert rc == 1
    out = capsys.readouterr().out
    assert "Use-case coverage gaps:" in out


def test_enterprise_readiness_write_defaults_recovers_missing_file(tmp_path, capsys):
    rc = enterprise_readiness.main(
...
def test_enterprise_readiness_emit_pack(tmp_path, capsys):
    (tmp_path / "docs").mkdir(parents=True)
    (tmp_path / "docs/use-cases-enterprise-regulated.md").write_text(
...
def test_enterprise_readiness_execute_writes_evidence(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(enterprise_readiness.subprocess, "run", lambda *a, **k: _Proc())
...
def test_main_cli_dispatches_enterprise_readiness(capsys):
