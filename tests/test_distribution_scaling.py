...
from sdetkit import distribution_scaling as d74
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-distribution-scaling-workflow.md\ndistribution-scaling-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-74-big-upgrade-report.md\nintegrations-distribution-scaling-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-distribution-scaling-workflow.md").write_text(
...
def test_distribution_scaling_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_distribution_scaling_top10_alignment_fails_when_markers_missing(
...
def test_distribution_scaling_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d74.main(
        [
...
def test_distribution_scaling_strict_fails_without_prior(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_distribution_scaling_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
