...
from sdetkit import kpi_deep_audit as d57
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-kpi-deep-audit-workflow.md\nkpi-deep-audit-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-57-big-upgrade-report.md\nintegrations-kpi-deep-audit-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-kpi-deep-audit-workflow.md").write_text(
...
def test_kpi_deep_audit_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_kpi_deep_audit_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d57.main(
        [
...
def test_kpi_deep_audit_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
...
def test_kpi_deep_audit_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
