...
def _seed_repo(root: Path) -> None:
    ...
    (root / "docs/integrations-day55-contributor-activation-closeout.md").write_text(d55._DAY55_DEFAULT_PAGE, encoding="utf-8")
    (root / "docs/day-55-big-upgrade-report.md").write_text("# Day 55 report\n", encoding="utf-8")

    summary = root / "docs/artifacts/day53-docs-loop-closeout-pack/day53-docs-loop-closeout-summary.json"
...
def test_day55_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    rc = d55.main(["--root", str(tmp_path), "--emit-pack-dir", "artifacts/day55-pack", "--execute", "--evidence-dir", "artifacts/day55-pack/evidence", "--format", "json", "--strict"])
    assert rc == 0
    assert (tmp_path / "artifacts/day55-pack/day55-contributor-activation-closeout-summary.json").exists()
    assert (tmp_path / "artifacts/day55-pack/day55-contributor-activation-closeout-summary.md").exists()
    ...
    assert (tmp_path / "artifacts/day55-pack/day55-contributor-activation-kpi-scorecard.json").exists()
...
def test_day55_strict_fails_without_day53(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    (tmp_path / "docs/artifacts/day53-docs-loop-closeout-pack/day53-docs-loop-closeout-summary.json").unlink()
    assert d55.main(["--root", str(tmp_path), "--strict", "--format", "json"]) == 1


def test_day55_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(["day55-contributor-activation-closeout", "--root", str(tmp_path), "--format", "text"])
    assert rc == 0
