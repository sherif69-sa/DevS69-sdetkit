...
from sdetkit import integration_feedback as d82
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-integration-feedback-workflow.md\nintegration-feedback-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-82-big-upgrade-report.md\nintegrations-integration-feedback-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-integration-feedback-workflow.md").write_text(
...
def test_integration_feedback_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_integration_feedback_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d82.main(
        [
...
def test_integration_feedback_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_integration_feedback_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
