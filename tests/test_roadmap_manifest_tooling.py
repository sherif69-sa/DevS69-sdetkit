...
from sdetkit.roadmap_manifest import _next_calls, build_manifest, check_manifest
...
def test_roadmap_manifest_includes_alignment_inventory() -> None:
    repo_root = Path(__file__).resolve().parents[1]
...
def test_next_calls_are_emitted_with_actionable_command() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    rows = _next_calls(repo_root=repo_root, limit=5)
    assert rows
    first = rows[0]
