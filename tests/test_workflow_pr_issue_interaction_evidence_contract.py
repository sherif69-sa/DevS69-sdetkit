...
def test_pr_issue_interaction_permission_evidence_matches_governance_summary() -> None:
    report = json.loads(
        Path("build/sdetkit/workflow-governance-report.json").read_text(encoding="utf-8")
    )
    evidence = json.loads(
        Path("build/sdetkit/workflow-permission-pr-issue-interaction-evidence.json").read_text(
            encoding="utf-8"
        )
    )

    groups = {group["kind"]: group for group in report["permission_review_summary"]["groups"]}
    target_group = groups["pr_issue_interaction"]

    assert evidence["schema_version"] == 1
    assert evidence["report_status"] == "review_required"
    assert evidence["evidence_type"] == "permission_group_evidence"
    assert evidence["permission_group"] == "pr_issue_interaction"
    assert evidence["workflow_count"] == target_group["workflow_count"] == 4
...
def test_pr_issue_interaction_permission_evidence_keeps_other_groups_separate() -> None:
    evidence = json.loads(
        Path("build/sdetkit/workflow-permission-pr-issue-interaction-evidence.json").read_text(
            encoding="utf-8"
        )
