...
def test_trend_delta_bootstrap_without_previous(tmp_path: Path) -> None:
    ...
    rc = trend_script.main(
        ["--current", str(current), "--out-json", str(out), "--skip-md", "--format", "json"]
    )
    assert rc == 0
...
def test_trend_delta_auto_uses_history_when_previous_not_passed(tmp_path: Path) -> None:
    current = _summary(
        tmp_path / "phase1-baseline-summary.json", [{"id": "doctor", "ok": False, "rc": 1}]
    )
    _summary(
        tmp_path / "history" / "phase1-baseline-summary-old.json",
        [{"id": "doctor", "ok": True, "rc": 0}],
    )
    out = tmp_path / "out.json"
