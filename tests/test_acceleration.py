...
from sdetkit import acceleration as d42
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-acceleration-workflow.md\nacceleration-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-43-big-upgrade-report.md\nintegrations-acceleration-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-acceleration-workflow.md").write_text(
...
def test_acceleration_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_acceleration_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d42.main(
        [
...
def test_acceleration_strict_fails_when_optimization_foundation_inputs_missing(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_acceleration_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
