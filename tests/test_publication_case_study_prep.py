...
from sdetkit import publication_case_study_prep as d72
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-case-study-prep4-workflow.md\ncase-study-prep4-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "-72-big-upgrade-report.md\nintegrations-case-study-prep4-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-case-study-prep4-workflow.md").write_text(
...
def test_case_study_prep4_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_case_study_prep4_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d72.main(
        [
...
def test_case_study_prep4_strict_fails_without_prior(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_case_study_prep4_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
