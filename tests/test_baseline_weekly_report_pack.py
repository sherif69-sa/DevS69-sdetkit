...
from scripts import baseline_weekly_report_pack as pack
...
def test_build_pack_payload(tmp_path: Path) -> None:
    ...
    _write(dashboard, {"ready_to_close": False, "next_step": "make baseline-next"})
...
def test_main_writes_outputs(tmp_path: Path) -> None:
    ...
    assert rc == 0
    assert (tmp_path / "pack" / "baseline-weekly-pack.json").exists()
    assert (tmp_path / "pack" / "baseline-weekly-pack.md").exists()
