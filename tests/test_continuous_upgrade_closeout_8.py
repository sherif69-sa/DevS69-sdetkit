...
from sdetkit import continuous_upgrade_closeout_8 as d98
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "docs/integrations-continuous-upgrade-closeout-8.md").write_text(
        d98._CYCLE8_DEFAULT_PAGE, encoding="utf-8"
    )
    (root / "docs/continuous-upgrade-big-upgrade-report-8.md").write_text(
        "# Cycle 8 report\n", encoding="utf-8"
    )
    (root / "scripts").mkdir(parents=True, exist_ok=True)
    (root / "scripts/check_continuous_upgrade_contract_8.py").write_text(
        "from __future__ import annotations\n\nif __name__ == 'main_':\n    raise SystemExit(0)\n",
        encoding="utf-8",
    )
    ...
    plan = root / "docs/roadmap/plans/continuous-upgrade-plan-8.json"
    plan.write_text(
        json.dumps(
            {
...
def test_json(tmp_path: Path, capsys) -> None:
    ...
    assert out["name"] == "continuous-upgrade-closeout-8"
...
def test_emit_pack_and_execute(tmp_path: Path) -> None:
    ...
    rc = d98.main(
        [
            ...
            str(tmp_path),
            "--emit-pack-dir",
            "artifacts/continuous-upgrade-closeout-8-pack",
            "--execute",
            "--evidence-dir",
            "artifacts/continuous-upgrade-closeout-8-pack/evidence",
            "--format",
            "json",
            "--strict",
        ]
    ...
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-closeout-8-summary.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-closeout-8-summary.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-evidence-brief.md"
    ).exists()
    assert (
        tmp_path / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-plan.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-upgrade-template-upgrade-ledger.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-storyline-outcomes-ledger.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-upgrade-kpi-scorecard.json"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-execution-log.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-delivery-board.md"
    ).exists()
    assert (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/continuous-upgrade-cycle8-validation-commands.md"
    ).exists()
    execution_summary = (
        tmp_path
        / "artifacts/continuous-upgrade-closeout-8-pack/evidence/cycle8-execution-summary.json"
    )
...
def test_strict_fails_without_cycle7(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    (
        tmp_path
        / "docs/artifacts/continuous-upgrade-closeout-pack-7/continuous-upgrade-closeout-summary-7.json"
...
def test_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(["continuous-upgrade-closeout-8", "--root", str(tmp_path), "--format", "text"])
    assert rc == 0
