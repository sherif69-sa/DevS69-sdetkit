...
from sdetkit import contributor_activation as d55
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-contributor-activation-workflow.md\ncontributor-activation-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-55-big-upgrade-report.md\nintegrations-contributor-activation-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-contributor-activation-workflow.md").write_text(
...
def test_contributor_activation_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_contributor_activation_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d55.main(
        [
...
def test_contributor_activation_strict_fails_without_prereq_baseline(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_contributor_activation_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
