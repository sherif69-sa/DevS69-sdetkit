...
from sdetkit import execution_prioritization as d50
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-execution-prioritization-workflow.md\nexecution-prioritization-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-50-big-upgrade-report.md\nintegrations-execution-prioritization-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-execution-prioritization-workflow.md").write_text(
...
def test_execution_prioritization_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_execution_prioritization_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d50.main(
        [
...
def test_execution_prioritization_strict_fails_when_weekly_review_inputs_missing(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_execution_prioritization_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = cli.main(
