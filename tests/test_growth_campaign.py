...
from sdetkit import growth_campaign as d81
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-growth-campaign-workflow.md\ngrowth-campaign-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-81-big-upgrade-report.md\nintegrations-growth-campaign-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-growth-campaign-workflow.md").write_text(
...
def test_growth_campaign_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_growth_campaign_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d81.main(
        [
...
def test_growth_campaign_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_growth_campaign_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
