...
from sdetkit import distribution as d36
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-distribution-workflow.md\ndistribution-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-36-big-upgrade-report.md\nintegrations-distribution-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-distribution-workflow.md").write_text(
...
def test_distribution_distribution_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_distribution_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d36.main(
        [
...
def test_distribution_strict_fails_when_prior_inputs_missing(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_distribution_strict_fails_when_prior_board_is_not_ready(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_distribution_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
