...
from sdetkit import scale_upgrade as d79
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-scale-upgrade-workflow.md\nscale-upgrade-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-79-big-upgrade-report.md\nintegrations-scale-upgrade-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-scale-upgrade-workflow.md").write_text(
...
def test_scale_upgrade_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_scale_upgrade_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d79.main(
        [
...
def test_scale_upgrade_strict_fails_without_ecosystem_priorities_baseline(
    tmp_path: Path,
) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_scale_upgrade_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
