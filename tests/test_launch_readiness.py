...
from sdetkit import launch_readiness as d86
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-launch-readiness-workflow.md\nlaunch-readiness-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-86-big-upgrade-report.md\nintegrations-launch-readiness-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-launch-readiness-workflow.md").write_text(
...
def test_launch_readiness_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_launch_readiness_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d86.main(
        [
...
def test_launch_readiness_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_launch_readiness_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
