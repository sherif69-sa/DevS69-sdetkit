...
from sdetkit import expansion as d45
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-expansion-workflow.md\nexpansion-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-45-big-upgrade-report.md\nintegrations-expansion-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-expansion-workflow.md").write_text(
...
def test_expansion_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_expansion_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d45.main(
        [
...
def test_expansion_strict_fails_when_scale_inputs_missing(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_expansion_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
