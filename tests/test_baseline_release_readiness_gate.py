...
from scripts import baseline_release_readiness_gate as gate
...
def test_build_gate_result_ready() -> None:
    ...
    assert payload["ready_for_release readiness"] is True
...
def test_main_success(tmp_path: Path) -> None:
    ...
    rc = gate.main(
        ["--readiness-signal", str(finish), "--artifact-set", str(artifacts), "--format", "json"]
    )
    assert rc == 0
