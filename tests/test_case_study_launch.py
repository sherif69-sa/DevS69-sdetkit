...
from sdetkit import case_study_launch as d73
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-case-study-launch-workflow.md\ncase-study-launch-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-73-big-upgrade-report.md\nintegrations-case-study-launch-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-case-study-launch-workflow.md").write_text(
...
def test_case_study_launch_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_case_study_launch_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d73.main(
        [
...
def test_case_study_launch_strict_fails_without_prior(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_case_study_launch_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
