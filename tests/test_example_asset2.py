...
def _seed_repo(root: Path) -> None:
    ...
    board = root / "docs/artifacts/example-asset-pack/example-delivery-board.md"
    board.write_text(
        "\n".join(
            [
                "#  delivery board",
...
def test_example_asset2_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_strict_fails_when_workflow_board_is_not_ready(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    (tmp_path / "docs/artifacts/example-asset-pack/example-delivery-board.md").write_text(
        "- [ ]  example asset #2 backlog pre-scoped\n", encoding="utf-8"
    )
    rc = d34.main(["--root", str(tmp_path), "--strict", "--format", "json"])
    assert rc == 1
...
def test_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(["example-asset2", "--root", str(tmp_path), "--format", "text"])
    assert rc == 0
    assert " example asset #2 summary" in capsys.readouterr().out
