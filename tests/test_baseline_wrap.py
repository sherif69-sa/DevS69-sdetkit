...
def _seed_repo(root: Path) -> None:
    ...
    (root / "docs/top-10-github-strategy.md").write_text(
        "- ** — Baseline wrap + handoff:** publish a full report and lock release readiness backlog.\n"
...
def test_baseline_wrap_wrap_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_baseline_wrap_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    rc = d30.main(
        [
            "--root",
...
def test_baseline_wrap_strict_fails_when_inputs_missing(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
...
def test_baseline_wrap_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(["baseline-wrap", "--root", str(tmp_path), "--format", "text"])
    assert rc == 0
