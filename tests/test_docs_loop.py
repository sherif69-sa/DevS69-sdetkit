...
from sdetkit import docs_loop as d53
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-docs-loop-workflow.md\ndocs-loop-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-53-big-upgrade-report.md\nintegrations-docs-loop-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-docs-loop-workflow.md").write_text(
...
def test_docs_loop_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_docs_loop_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d53.main(
        [
...
def test_docs_loop_strict_fails_when_narrative_inputs_missing(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
...
def test_docs_loop_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
