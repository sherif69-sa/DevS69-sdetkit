...
from sdetkit import community_program as d62
...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-community-program-workflow.md\ncommunity-program-closeout\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-62-big-upgrade-report.md\nintegrations-community-program-workflow.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-community-program-workflow.md").write_text(
...
def test_community_program_json(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
...
def test_community_program_emit_pack_and_execute(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    rc = d62.main(
        [
...
def test_community_program_strict_fails_without_prereq_baseline(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
    (
        tmp_path
...
def test_community_program_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    seed_contract_anchors(tmp_path)
